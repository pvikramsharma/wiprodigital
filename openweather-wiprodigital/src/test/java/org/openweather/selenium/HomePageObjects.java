...
public class HomePageObjects extends DriverManager {
	...
	// this function will return either current date dd or current Hour HH based on
	// the parameter date or time
	...
	// based on first forecast date and if is not matching with the current date
	// then return false
	public Boolean getWeatherForecastDate() throws Exception {

		if (getCurrentDateorTime("date").equalsIgnoreCase(
				driver.findElement(By.xpath("//*[@id='root']/div/div[1]/div[1]/span[1]/span[2]")).getText())) {
			return true;
		}
